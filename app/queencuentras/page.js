    ...
    <div className="p-5 max-w-screen-xl mx-auto">
      ...
      {categories.map((category, index) => (
        ...
        >
          ...
          <div className="sm:max-w-[50vh] flex flex-col justify-center">
            <TextComponent className="w-full sm:w-1/2">
              <h2 className="text-2xl font-bold mb-3">{category.name}</h2>
              <p className="text-gray-700">{category.description}</p>
            </TextComponent>
            <Button onClick={()=>router.push(category.link)} className="w-1/2 m-auto">Click aqui para ver más</Button>
          </div>
        </div>
      ))}
    </div>
