...
const ListaAlojamientos = ({ alojamientos, onViewDetails }) => {
  ...
  return (
    <List itemLayout="horizontal" dataSource={alojamientos} renderItem={(alojamiento) => ( <List.Item>
      <Card hoverable style={{ width: '100%', padding: '16px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', borderRadius: '8px', display: 'flex', alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap', 
      backgroundColor: '#ffffff'}}
        cover={alojamiento.images && alojamiento.images.length > 0 ? (
          <img alt={alojamiento.title || 'Imagen del alojamiento'} 
          src={alojamiento.images[0]?.url || 'https://via.placeholder.com/600x400'} 
          style={{ width: 300, height: 200, objectFit: 'cover', marginRight: 16 }}/>) : null } >
            
    <div style={{ flex: 1 }}>
      <Meta title={alojamiento.title || 'Título no disponible'} 
        description={
    ...
    <div>{alojamiento.caracteristicas?.comodidades?.length > 0 ? (
  ...
  />
    ...
    <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'flex-start' }}>
      <Button type="primary" onClick={() => onViewDetails(alojamiento.id)} style={{ marginTop: '16px' }}>Ver Detalles </Button></div>
      </Card>
</List.Item>
      )}
    />
